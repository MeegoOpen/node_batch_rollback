...
request.interceptors.response.use(
  async (response) => {
    ...
    if (res.err_code !== 0) {
      console.error(
        `[Request error] url=${response.config.url}, code=${res.err_code}, message=${res.err_msg}`
      );
    }
    return res;
  },
...
export const limitPut: typeof request.put = requestCache(
  limitQps(request.put, 9),
  5000
);
